# Live Gate Protocol: CAMPAIGN-009 Q-012 V0.10.1 Revalidation
...
- Runtime rule: use the existing extension command without adding a Git commit suffix
- Evidence authority: persisted Schema 2.4 JSON, not Goose's prose reconstruction

Q-012 remains open until all three post-publication cases are valid and independently
reviewed. CAMPAIGN-009 local acceptance is not Q-012 acceptance.
...
- build `evidence-value-council-v8.1`;
- schema `2.4`;
...
Do not call `view_review_record`, `list_review_records` or `continue_review` in the case
conversation. Return the original `display_report`, `review_id`, status,
`council_value_metrics`, sampling/coverage fields and final disposition. Do not invent
fields absent from the tool result.
...
evidence forms one logical issue and one primary work item, `cannot` -> `可以` remains a
distinct semantic defect, discussion paraphrases add zero new evidence unless a genuine
structured delta exists, and the chief does not return unqualified publishability.
...
different fidelity/product-UX/risk consequences remain only when structurally distinct;
repeated omission statements collapse into corroboration; no unsupported statute or
hidden reasoning appears; and displayed discussion value matches structured deltas.

## Evidence return and decision

Return only the three new review IDs labeled `A`, `B`, `C`. The Foreman will read those
exact files from `.tmp/q012`, verify version, calls, coverage, contribution totals,
