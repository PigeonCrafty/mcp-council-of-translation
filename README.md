...
Council of Translation V0.10.2 is a Goose-first, review-only MCP server for context-coherent localization QA. It reviews a source/candidate pair, returns a concise Council report as the primary MCP text, and retains the complete existing dictionary as structured content. It never translates files or applies edits: the caller supplies relevant terminology, style, project, and technical context and owns the final edit.
...
The pinned build reports version `0.10.2`, schema `2.4`, diagnostic build `evidence-value-council-v8.2`, and budgets 6/13/18.
