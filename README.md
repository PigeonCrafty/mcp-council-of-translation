...
For the normal-user Q-009 Goose revalidation, replace `<reviewed-commit>` above with the exact accepted commit, provide only the source and candidate, answer the brief background form when it appears, and ask Goose to show the Council review. Audit the first primary text block as the user-facing answer: it should directly show all six role lenses, truthful consensus or uncertainty, material risks and the verdict last, without procedural counters or redundant clean-role evidence. Then inspect the same tool result's structured content—not a second history call—to confirm the complete chief rationale and role evidence remain available.

The pinned build reports version `0.7.1`, schema `2.2`, diagnostic build `concise-council-display-v5.1`, and budgets 6/13/18.
